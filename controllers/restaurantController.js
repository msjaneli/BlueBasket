...
exports.updateImage = async (req, res) => {
  ...
  const resizedImageUrl = await cloudinary.url(
    "restaurant_image/" + name.split(" ").join("%20") + ".jpg",
    {
      version: version,
      quality: 100,
      height: 400,
...
exports.register = async (req, res) => {
  ...
  if (!isEmpty(rows)) {
    return res
      ...
      .json({
        error:
          "There is already a restaurant registered under this email address"
      });
  }
  ...
  return res
    .status(200)
    .send({ success: "Successfully registered new restaurant" });
};
